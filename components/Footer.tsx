    ...
    <footer style={{ background: '#000000', borderTop: '1px solid rgba(255,255,255,0.06)', padding: 'clamp(48px, 8vw, 80px) 0 36px' }}>
      <div className="container-padded" style={{ maxWidth: 1440, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <a href="#" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 7.5, fontWeight: 500, color: '#e5e7eb', letterSpacing: '0.22em', textTransform: 'uppercase', marginTop: 2 }}>MARKETS</div>
              </div>
            </a>
            <p style={{ fontSize: 14, color: '#e5e7eb', lineHeight: 1.65, maxWidth: 260 }}>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 28 }}>
          <p style={{ fontSize: 11, color: '#a1a1aa', lineHeight: 1.8, marginBottom: 24 }}>
            <strong style={{ color: '#e5e7eb' }}>Risk Warning:</strong> Trading leveraged products such as Forex and CFDs carries a high degree of risk to your capital and may not be suitable for all investors. Please ensure you fully understand the risks involved before trading. Past performance is not indicative of future results.
          </p>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 20 }}>
              {['Twitter', 'LinkedIn', 'Telegram'].map(s => (
                <motion.a 
                  key={s} href="#" 
                  whileHover={{ scale: 1.1, color: '#fff', textShadow: '0 0 10px rgba(45,96,255,0.8)' }}
                  style={{ fontSize: 13, color: '#e5e7eb', textDecoration: 'none', transition: 'color 0.2s', fontWeight: 500 }}
                >{s}</motion.a>
              ))}
            </div>
          </div>
  ...
  return (
    <div>
      ...
      <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
        {links.map(l => (
          <li key={l}>
            <a href="#" style={{ fontSize: 14, color: '#e5e7eb', textDecoration: 'none', transition: 'color 0.2s', fontWeight: 500 }}
